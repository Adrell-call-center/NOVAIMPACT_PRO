...
export default function AdminPostEdit() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/posts/${id}`).then(r => r.json()).then(d => {
      if (d.post) {
        setForm({ ...d.post, tags: (d.post.tags || []).join(', '), schemaOverridesRaw: d.post.schemaOverrides ? JSON.stringify(d.post.schemaOverrides, null, 2) : '' });
      }
      setLoading(false);
    });
    fetch('/api/admin/uploads').then(r => r.json()).then(d => setUploads(d.uploads || []));
  ...
  return (
    <>
      <Head><title>Edit Post — Nova Impact Admin</title></Head>
      <AdminLayout title={`Edit: ${form.titleFr.substring(0, 30)}${form.titleFr.length > 30 ? '...' : ''}`}>
        ...
        {/* tabs removed — all panels now in 2-col layout */}
          ...
          <div className="admin-col-left editor-scroll">
            <div className="admin-light-card admin-editor-card">
              <div className="form-group mb-0">
                ...
                <input className="admin-input admin-input-lg" value={form.titleFr} onChange={e => { update('titleFr', e.target.value); update('metaTitleFr', e.target.value); }} placeholder="Enter post title" />
              </div>
            ...
            <div className="admin-quill-card mt-4">
              <RichTextEditor defaultValue={form.contentFr} onChange={(val) => update('contentFr', val)} onImageUpload={async (file) => {
              ...
              }} placeholder="Write your post content..." minHeight={600} />
            </div>
      ...
      <style jsx global>{`
        ...
        .admin-col-left::-webkit-scrollbar-thumb { background: #d0d0d0; border-radius: 3px; }
