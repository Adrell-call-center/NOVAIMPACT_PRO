                  ...
                  <div className="admin-editor-lang-content">
                    {langTab === 'fr' ? (
                      <div className="form-group mb-0">
                        <RichTextEditor value={form.contentFr} onChange={(val) => update('contentFr', val)} uploads={uploads} onUpload={async (file) => {
                          const fd = new FormData();
                          fd.append('file', file);
                          const res = await fetch('/api/admin/uploads', { method: 'POST', body: fd });
                          const data = await res.json();
                          return data.url;
                        }} placeholder="Write your post content in French..." />
                      </div>
                    ) : (
                      <div className="form-group mb-0">
                        <RichTextEditor value={form.contentEn} onChange={(val) => update('contentEn', val)} uploads={uploads} onUpload={async (file) => {
                          const fd = new FormData();
                          fd.append('file', file);
                          const res = await fetch('/api/admin/uploads', { method: 'POST', body: fd });
                          const data = await res.json();
